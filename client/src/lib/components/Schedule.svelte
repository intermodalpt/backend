...
<div class="flex flex-col w-full h-full">
  <div class="p-2 bg-primary flex flex-row items-center gap-1 h-12 lg:h-16 w-full">
    <div class="rounded-full btn btn-ghost fill-primary-content w-12 px-3" on:click={back}>
      <span class="text-2xl">🡸</span>
    </div>
    <span class="text-lg font-bold text-primary-content whitespace-nowrap overflow-hidden">
      {$scheduleId}
    </span>
  </div>
  ...
  <div class="p-2 overflow-y-scroll flex flex-col gap-2">
    <div class="flex flex-col gap-2 pt-2">
      {#each Object.keys(schedule) as key}
        ...
        <div class="h-[2px] bg-base-300 w-full rounded-full" />
      {/each}
    </div>
  </div>
</div>
