<script>
  ...
  import Operators from "./lib/Operators.svelte";
...
<div class="w-[min(920px,100%)] mx-auto z-[3000] p-2 pb-4 xl:pt-4">
  <div class="navbar bg-base-100 shadow-xl rounded-xl">
    <div class="navbar-start">
      <div class="">
        ...
        <ul
          ...
          {#each links as k}
            <li><a class={k[0] === path ? "bg-primary text-primary-content" : ""} href="#/{k[0]}">{k[1]}</a></li>
          {/each}
        </ul>
      </div>
    ...
    <div class="navbar-end hidden lg:flex">
      <ul class="menu menu-horizontal p-0">
        {#each links as k}
          <li><a class={k[0] === path ? "bg-base-300" : ""} href="#/{k[0]}">{k[1]}</a></li>
        {/each}
      </ul>
    </div>
  </div>
