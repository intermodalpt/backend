function converteDeMunicipioLinhaAntiga() {
    ...
    a += "</select>", a += "</div>", document.getElementById("divSeletorMunicipios").innerHTML = a, document.querySelector("#seletorMunicipios").onchange = function () {
        ...
        for (const e in diretorio[a]) "Nova" != e && (o += `<option value="${e}">${e}</option>`);
        ...
        a = 1 == diretorio[e][o].length ? "Nova linha correspondente:<br>" : "Novas linhas correspondentes:<br>";
        for (let i in diretorio[e][o]) {
            a += "<div class='novaLinha'>", a += `    <div class="novaLinhaNum">${diretorio[e][o][i][0]}</div>`, a += `    <div class="novaLinhaNome">${diretorio[e][o][i][1]}</div>`, a += "</div>"
        }
...
function converteDeOperadorLinhaAntiga() {
    ...
    a += "</select>", a += "</div>", document.getElementById("divSeletorOperadores").innerHTML = a, document.querySelector("#seletorOperadores").onchange = function () {
        ...
        for (const e in diretorio_operadores[a]) "Nova" != e && (o += `<option value="${e}">${e}</option>`);
        ...
        a = 1 == diretorio_operadores[e][o].length ? "<b>Nova linha correspondente</b><br>" : "<b>Novas linhas correspondentes</b><br>";
        for (let i in diretorio_operadores[e][o]) {
            a += "<div class='novaLinha'>", a += `    <div class="novaLinhaNum">${diretorio_operadores[e][o][i][0]}</div>`, a += `    <div class="novaLinhaNome">${diretorio_operadores[e][o][i][1]}</div>`, a += "</div>"
        }
...
function listaNovasLinhasDeMunicipio() {
    ...
    a += "</select></div></div>", a += '<div id="listaNovasLinhas"></div>', document.querySelector("#novasLinhasMunicipio").innerHTML = a, document.querySelector("#seletorNovasLinhasMunicipio").onchange = function () {
        ...
        for (let e in diretorio[a].Nova) {
            o += "<div class='novaLinha'>", o += `    <div class="novaLinhaNum">${diretorio[a].Nova[e][0]}</div>`, o += `    <div class="novaLinhaNome">${diretorio[a].Nova[e][1]}</div>`, o += "</div>"
        }
...
function pesquisaNovaCarreira() {
    ...
    if (0 == e) i = ""; else {
        var i;
        i = 1 == e ? "<p>Nova linha correspondente:</p>" : "<p>Novas linhas correspondentes:</p>";
...
function listaLinhasDeMunicipio() {
    ...
    a += "</select></div></div>", a += '<div id="idListaLinhas" style="display:none"></div>', document.querySelector("#idLinhasMunicipio").innerHTML = a, document.querySelector("#idLinhaEscolhida").onchange = function () {
        let a = document.querySelector("#idLinhaEscolhida").value, o = "";
        for (let e in diretorio_linhas_por_municipio[a]) {
            let i = diretorio_linhas_por_municipio[a][e][0];
            o += "<div class='novaLinha'>", o += `    <div class="novaLinhaNum" style='background-color:${da_cor(e)}'>${e}</div>`, o += `    <div class="novaLinhaNome">${i}</div>`, o += `    <button class="horario" value='${e}' style='background-color:${da_cor(e)}'>horÃ¡rio</button>`, o += "</div>"
        }
        document.getElementById("idListaLinhas").innerHTML = o, document.getElementById("idListaLinhas").style.display = "block", document.querySelectorAll("#idListaLinhas button").forEach((a => {
            a.onclick = () => {
                document.querySelector("#inputLinha").value = a.value, mostraLinha(), document.querySelector("#novasCarreirasPorMunicipioContent").style.display = "none", document.querySelector("#novasCarreirasPorMunicipio").setAttribute("class", "accordion__btn js-accordion-btn"), document.querySelector("#novasCarreirasPorMunicipio").parentElement.setAttribute("class", ""), document.querySelector("#horariosContent").style.display = "block", document.querySelector("#horariosSeparador").setAttribute("class", "accordion__btn js-accordion-btn active"), document.querySelector("#horariosSeparador").parentElement.setAttribute("class", "active")
            }
        }))
    }
}
...
jQuery(document).ready((function () {
    ...
    converteDeMunicipioLinhaAntiga(), converteDeOperadorLinhaAntiga(), listaNovasLinhasDeMunicipio(), listaLinhasDeMunicipio()
}));
...
function mostraLinha() {
    var a = document.querySelector("#inputLinha").value;
    if (document.querySelector("#infolinha").style.display = "flex", !(a in linhas_e_seus_horarios_nomes_ficheiros) || 0 == Object.keys(linhas_e_seus_horarios_nomes_ficheiros[a]).length) return document.querySelector("#paragens").innerHTML = "", document.querySelector("#variante").innerHTML = `NÃ£o existe horÃ¡rio para a linha ${a}.`, document.querySelector("#infolinha").style.display = "none", document.querySelector("#comentarioInfo").style.display = "none", 0;
    switch (linhas_e_tarifario[a]) {
        case"RÃ¡pida":
            "#ffb005";
            break;
    ...
    const o = document.querySelector("#variante");
    var e = !0;
    for (const i in linhas_e_seus_horarios_nomes_ficheiros[a]) {
        const n = document.createElement("label"), s = linhas_e_seus_horarios_nomes_ficheiros[a][i],
            r = document.createElement("input");
        r.setAttribute("type", "radio"), r.setAttribute("name", "rota"), r.setAttribute("value", s), n.appendChild(r);
        const l = document.createElement("div");
        l.innerHTML = i, l.style.marginLeft = "5px", l.style.fontWeight = "normal", n.appendChild(l);
        const t = s.slice(0, -5);
        if (t in horarios_pdf) {
            ...
            e.setAttribute("src", "images/save_to_pdf.png"), o.appendChild(e)
        }
        o.appendChild(n), e && (r.setAttribute("checked", !0), l.style.fontWeight = "bold", criaHorario(s), e = !1)
    }
...
function criaHorario(a) {
    document.querySelector("#comentarioInfo").style.display = "block", document.querySelector("#meuHorarioEscolhas").innerHTML = "", document.querySelector("#paragens").style.display = "block", criaHorarioProximo(a)
}
...
function criaHorarioProximo(a) {
    var o, e, i, n, s, [r, l] = mostraHoraEDiaNosInputs();
    document.getElementById("horario").innerHTML = "", r < 20220701 && !(a.slice(0, 3) >= "440" && a.slice(0, 3) < "447" || "4551" == a.slice(0, 4)) ? document.querySelector("#comentarioInfo").innerHTML = "Clique na paragem e consulte o horÃ¡rio de passagem (vÃ¡lido a partir de 1 julho)" : r < 20220704 && (a.slice(0, 3) >= "440" && a.slice(0, 3) < "447" || "4551" == a.slice(0, 4)) ? document.querySelector("#comentarioInfo").innerHTML = "Clique na paragem e consulte o horÃ¡rio de passagem (vÃ¡lido a partir de 4 julho)" : document.querySelector("#comentarioInfo").innerHTML = "Clique na paragem e consulte o horÃ¡rio de passagem", meuHorarioEscolhas.innerHTML = "", fetch("images/horarios/" + a).then((a => a.json())).then((t => {
        ...
        document.querySelector("#inputLinha").style.backgroundColor = s, document.querySelector("#inputLinha").style.border = "";
        document.getElementById("horario").innerHTML = "";
        const p = cria_horario(t, 0);
        document.getElementById("horario").appendChild(p), mostra_legenda_periodos();
        var _ = 0;
        ...
        t.forEach((t => {
            ...
            if (c.appendChild(p), (d = document.createElement("td")).setAttribute("class", "tempo"), d.style.fontSize = "0.9em", m) {
                n = 0, o = parseInt(t[1][0][0].replace(":", ""), 10), e = 0;
                let a = document.createElement("img");
                a.setAttribute("src", "images/clock.png"), d.appendChild(a)
            }
            i = parseInt(t[1][0][0].replace(":", ""), 10) - o, e += i, 0 != n++ && n % 4 == 0 && (d.innerHTML = "<hr>", document.querySelector(`#espinha tr:nth-child(${n - 2}) td:first-child`).innerHTML = e, e = 0), o = parseInt(t[1][0][0].replace(":", ""), 10), p.appendChild(d);
            var d = document.createElement("td");
            if (d.setAttribute("class", "cor"), d.style.backgroundColor = s, m) {
                d.style.borderRadius = "1em 1em 0 0";
                var u = document.createElement("img");
                u.setAttribute("src", "images/bullet-start.png"), u.style.width = "1.4em", u.style.verticalAlign = "center", d.appendChild(u);
                const a = document.createElement("span");
                a.setAttribute("class", "bola"), d.appendChild(a)
            } else {
                const a = document.createElement("span");
                a.setAttribute("class", "bola");
                var h = document.createElement("img");
                h.setAttribute("src", "images/bullet-current.png"), h.style.width = "1.4em", h.style.verticalAlign = "center", a.appendChild(h), d.appendChild(a)
            }
            p.appendChild(d), (d = document.createElement("td")).setAttribute("class", "hr");
            const S = document.createElement("hr");
            d.appendChild(S), p.appendChild(d), (d = document.createElement("td")).setAttribute("class", "paragem paragemProximo"), d.setAttribute("data-paragem", "" + _++), p.appendChild(d);
            const b = document.createElement("div");
            b.setAttribute("class", "paragemInfo"), d.append(b);
            const L = document.createElement("span");
            L.setAttribute("class", "nome");
            const v = document.createTextNode(t[0]);
            L.appendChild(v), b.appendChild(L), m && (m = !1);
            var g = [];
            for (let a = 0; a < t[1].length; a++) {
                var C = parseInt(t[1][a][0].replace(":", ""));
                date_service_ids[r].includes("" + t[1][a][1]) && C > l && g.push(t[1][a][0])
            }
            g.sort((function (a, o) {
            ...
            var A;
            P.setAttribute("class", "proxima"), P.style.fontSize = "0.85em", g.length > 0 ? (proxima_passagem = g[0], A = document.createTextNode("PrÃ³ximo: " + g[0])) : A = r < 20220701 && "3" == a[0] ? document.createTextNode("HorÃ¡rio vÃ¡lido sÃ³ a partir de 1 julho.") : document.createTextNode("hoje jÃ¡ nÃ£o circulam mais autocarros desta linha"), P.appendChild(A), T.appendChild(P);
            const M = document.createElement("span");
            M.setAttribute("class", "seguintes"), M.style.fontSize = "0.85em", M.style.padding = "0 0 0 1em";
            var j = document.createTextNode("");
            1 == g.length ? j = document.createTextNode("(Ãºltimo)") : 2 == g.length ? j = document.createTextNode(`Seguinte: ${g[1]}`) : g.length > 2 && (j = document.createTextNode(`Seguintes: ${g[1]}, ${g[2]}`)), M.appendChild(j), T.appendChild(M), d.appendChild(T)
        }));
        var u = document.createElement("img");
        u.setAttribute("src", "images/bullet-end.png"), u.style.verticalAlign = "center", u.style.width = "1.4em", c.querySelector("tr:last-child td:nth-child(2) .bola").innerHTML = "", c.querySelector("tr:last-child td:nth-child(2)").appendChild(u), c.querySelector("tr:last-child td:nth-child(2)").style.borderRadius = "0 0 1em 1em", n % 4 > 2 ? document.querySelector(`#espinha tr:nth-child(${n - 2}) td:first-child`).innerHTML = e : n % 4 > 1 && (document.querySelector(`#espinha tr:nth-child(${n - 1}) td:first-child`).innerHTML = e), document.querySelectorAll(".paragem").forEach((a => {
            a.parentElement.querySelector(".bola").style.visibility = "hidden"
        })), abrePrimeiraParagem(s), document.querySelectorAll("#espinha > *").forEach((a => {
            a.onmouseover = () => {
                "bold" != a.querySelector(".nome").style.fontWeight && (a.childNodes[3].style.backgroundColor = s, a.childNodes[3].style.opacity = "0.7", a.childNodes[3].style.borderRadius = "1em", a.querySelector(".bola").style.visibility = "visible", a.querySelector(".bola").style.opacity = "0.2")
            }, a.onmouseout = () => {
                "bold" != a.querySelector(".nome").style.fontWeight && (a.childNodes[3].style.backgroundColor = "", a.childNodes[3].style.opacity = "1", a.querySelector(".bola").style.visibility = "invisible", a.querySelector(".bola").style.opacity = "0.0")
            }
        })), document.querySelectorAll(".paragemProximo").forEach((a => {
            a.onclick = () => {
                document.querySelectorAll(".paragemProximo").forEach((a => {
                    a.parentElement.querySelector(".bola").style.visibility = "hidden", a.style.backgroundColor = "", a.querySelector(".horario").style.display = "none", a.querySelector(".nome").style.fontWeight = "normal"
                })), a.parentElement.querySelector(".bola").style.visibility = "visible", a.parentElement.querySelector(".bola").style.opacity = "1.0", a.style.backgroundColor = s, a.style.opacity = "1.0", a.querySelector(".nome").style.fontWeight = "bold", a.querySelector(".nome").style.whiteSpace = "wrap", a.style.borderRadius = "1em", a.querySelector(".horario").style.display = "block", document.getElementById("horario").innerHTML = "";
                const o = cria_horario(t, parseInt(a.dataset.paragem, 10));
                document.getElementById("horario").appendChild(o), mostra_legenda_periodos()
            }
        }))
    }))
}
...
function cria_horario(a, o) {
    const e = a[o][1], i = [[], [], [], [], [], [], [], [], []];
    for (let a = 0; a < e.length; a++) {
        const o = e[a][0], n = e[a][1];
        for (let a = 0; a < 9; a++) "0" != plano_oferta[n][a] && i[a].push(o)
    }
    const n = [];
    ...
    for (let a = 0; a < n.length; a++) for (let o = 0; o < 26; o++) n[a][o].sort();
    const s = [];
    for (let a = 0; a < n.length; a++) {
        var r = 0;
        for (let o = 0; o < 26; o++) n[a][o].length > r && (r = n[a][o].length);
        const o = [];
        for (let a = 0; a < 26; a++) {
            const a = [];
            for (let o = 0; o < r; o++) a.push(" ");
            o.push(a)
        }
        for (let e = 0; e < n[a].length; e++) for (let i = 0; i < n[a][e].length; i++) o[e][i] = n[a][e][i];
        s.push(o)
    }
    var l, t;
    const p = ["PerÃ­odo Escolar", "PerÃ­odo FÃ©rias Escolares", "PerÃ­odo de VerÃ£o"],
        _ = ["Dias Ãšteis", "SÃ¡bados", "Domingos"], d = document.createElement("div");
    d.setAttribute("id", "divGlobal");
    var c = {};
    for (let a = 0; a < s.length; a++) {
        if (0 == s[a][0].length) continue;
        const o = document.createElement("div");
        if (d.appendChild(o), a % 3 == 0) {
            const e = document.createElement("div");
            e.setAttribute("class", "periodo"), l = p[Math.floor(a / 3)], texto = document.createTextNode(l), e.appendChild(texto), o.appendChild(e)
        }
        const e = document.createElement("div");
        e.setAttribute("class", "tipodia"), t = _[a - 3 * Math.floor(a / 3)], hr = document.createElement("hr"), e.appendChild(hr), texto = document.createTextNode(t), e.appendChild(texto), o.appendChild(e);
        ...
        m.innerHTML = "Hora", m.style.width = "4.5ch", m.style.padding = "0 1ch", m.style.borderRadius = "1ch 0 0 1ch", m.style.textAlign = "left", n.appendChild(m);
        for (let a = 4; a <= 24; a++) (m = document.createElement("th")).innerHTML = a, n.appendChild(m);
        (m = document.createElement("th")).innerHTML = 1, n.appendChild(m), (m = document.createElement("th")).innerHTML = 2, n.appendChild(m), (m = document.createElement("th")).innerHTML = 3, n.appendChild(m), m.style.paddingRight = "1ch", m.style.fontWidth = "3.5ch", m.style.borderRadius = "0 1ch 1ch 0", i.appendChild(n);
        var u = !0;
        for (let o = 0; o < s[a][0].length; o++) {
            const e = document.createElement("tr");
            for (let i = 0; i < 26; i++) {
                const n = document.createElement("td");
                if (u) texto = document.createTextNode("Min."), u = !1, n.appendChild(texto), n.style.textAlign = "left", n.style.paddingLeft = "1ch", e.appendChild(n); else {
                    const r = s[a][i][o], l = r[r.length - 1];
                    /^[A-Z]$/i.test(l) ? (texto = document.createTextNode(s[a][i][o].slice(0, 2)), n.appendChild(texto), sup = document.createElement("sup"), texto = document.createTextNode(l), sup.appendChild(texto), n.appendChild(sup), c[l] = plano_oferta_legenda[l]) : (texto = document.createTextNode(s[a][i][o]), n.appendChild(texto)), e.appendChild(n)
                }
            }
            i.appendChild(e)
        }
    ...
    if (0 != Object.keys(c).length) {
        ...
        for (letra in c) {
            ...
            a.appendChild(o), h.appendChild(a);
            const e = document.createElement("span");
            e.innerHTML = c[letra] + "; ", h.appendChild(e)
        }
        d.appendChild(h)
    }
    var S = document.createElement("div");
    return S.setAttribute("class", "descricao_periodos"), S.innerHTML = '<span class="periodos_info">Datas dos perÃ­odos escolar, fÃ©rias escolares e verÃ£o:</span><span class="periodos_datas">   - <b>Periodo Escolar</b>: 2022: 3.1-7.4, 22.4-23.6, 16.9-16.12 | 2023: 3.1-17.2, 23.2-24.3, 10.4-18.6<br>   - <b>Periodo de FÃ©rias Escolares</b>: 2022: 1-3.1, 8-21.4, 24-30.6, 1-15.9, 17-31.12 | 2023: 1-3.1, 18-23.2, 25.3-9.4, 19-30.6<br>   - <b>Periodo de VerÃ£o</b>: 1.7-31.8<br>', d.appendChild(S), d
}
...
function verificaSeEnter(a) {
    "Enter" == a.key && mostraLinha()
}

document.addEventListener("DOMContentLoaded", (() => {
    var [a, o] = mostraHoraEDiaNosInputs();
    mostraSelectLinhas(), document.querySelector("#tipoHorario").innerHTML = "meuHorario", mostraBotoes(), document.querySelector("#inputLinha").onchange = mostraLinha
}));
const linhas_e_seus_horarios_nomes_ficheiros = {
        ...
        4310: {"Ã�guas de Moura - PoceirÃ£o": "4310_0_1.json", "PoceirÃ£o - Ã�guas de Moura": "4310_0_2.json"},
        ...
        4706: {"SÃ£o Francisco - Lisboa (Oriente)": "4706_0_1.json", "Lisboa (Oriente) - SÃ£o Francisco": "4706_0_2.json"},
        4707: {
        ...
        4720: {"Lisboa (Oriente) - SetÃºbal (ITS)": "4720_0_1.json", "SetÃºbal (ITS) - Lisboa (Oriente)": "4720_0_2.json"},
        4725: {
        ...
        "3111_0_2": "Linha_3111_140074_R_2022-06-23-16-35.pdf",
        "3111_0_1": "Linha_3111_140555_G_2022-06-23-16-35.pdf",
        ...
        "3218_0_2": "Linha_3218_150051_R_2022-06-23-16-42.pdf",
        "3218_0_1": "Linha_3218_150415_G_2022-06-23-16-42.pdf",
        ...
        "3513_0_1": "Linha_3513_020959_G_2022-06-23-16-49.pdf",
        "3513_0_2": "Linha_3513_140637_R_2022-06-23-16-49.pdf",
        ...
        "3527_0_1": "Linha_3527_020317_G_2022-06-23-23-01.pdf",
        "3527_0_2": "Linha_3527_140012_R_2022-06-23-23-01.pdf",
        ...
        "3626_0_1": "Linha_3626_140857_G_2022-06-23-16-55.pdf",
        "3626_0_2": "Linha_3626_160775_R_2022-06-23-16-55.pdf",
        "3635_0_1": "Linha_3635_140027_G_2022-06-23-16-56.pdf",
        "3635_0_2": "Linha_3635_150052_R_2022-06-23-16-56.pdf",
        "3640_0_1": "Linha_3640_150170_G_2022-06-23-16-56.pdf",
        "3640_0_2": "Linha_3640_160966_R_2022-06-23-16-56.pdf",
        ...
        "3026_0_1": "Linha_3026_020533_G_2022-06-23-16-29.pdf",
        "3026_0_2": "Linha_3026_020758_R_2022-06-23-16-30.pdf",
        "3027_0_2": "Linha_3027_020617_R_2022-06-23-16-30.pdf",
        "3027_0_1": "Linha_3027_020755_G_2022-06-23-16-30.pdf",
        ...
        "4401_0_1": "Linha_4401_160012_G_2022-06-29-16-20.pdf",
        "4401_0_2": "Linha_4401_160031_R_2022-06-29-16-20.pdf",
        "4402_0_1": "Linha_4402_160042_G_2022-06-29-16-13.pdf",
        "4402_0_2": "Linha_4402_160067_R_2022-06-29-16-13.pdf",
        "4403_0_1": "Linha_4403_160073_G_2022-06-29-16-22.pdf",
        "4403_0_2": "Linha_4403_160101_R_2022-06-29-16-22.pdf",
        "4405_0_1": "Linha_4405_160143_G_2022-06-29-16-24.pdf",
        "4405_0_2": "Linha_4405_160189_R_2022-06-29-16-24.pdf",
        "4406_0_1": "Linha_4406_160051_G_2022-06-29-15-55.pdf",
        "4406_0_2": "Linha_4406_160161_R_2022-06-29-15-55.pdf",
        "4407_0_1": "Linha_4407_160051_G_2022-06-29-16-08.pdf",
        "4408_0_2": "Linha_4408_160161_R_2022-06-29-15-55.pdf",
        "4409_0_1": "Linha_4409_160237_G_2022-06-29-16-18.pdf",
        "4409_0_2": "Linha_4409_160268_R_2022-06-29-16-18.pdf",
        "4410_0_1": "Linha_4410_160050_G_2022-06-29-16-16.pdf",
        "4410_0_2": "Linha_4410_160371_R_2022-06-29-16-16.pdf",
        "4411_0_2": "Linha_4411_160067_R_2022-06-29-16-14.pdf",
        "4411_0_1": "Linha_4411_160305_G_2022-06-29-16-14.pdf",
        ...
        "4421_0_1": "Linha_4421_160419_G_2022-06-29-16-04.pdf",
        "4421_0_2": "Linha_4421_160451_R_2022-06-29-16-04.pdf",
        "4422_0_1": "Linha_4422_160419_G_2022-06-29-16-03.pdf",
        "4422_0_2": "Linha_4422_160464_R_2022-06-29-16-03.pdf",
        "4423_0_2": "Linha_4423_160101_R_2022-06-29-16-23.pdf",
        "4423_0_1": "Linha_4423_160467_G_2022-06-29-16-23.pdf",
        ...
        "4427_0_2": "Linha_4427_160015_R_2022-06-29-16-19.pdf",
        "4427_0_1": "Linha_4427_160161_G_2022-06-29-16-19.pdf",
        "4428_0_2": "Linha_4428_160293_R_2022-06-29-16-04.pdf",
        "4428_0_1": "Linha_4428_160464_G_2022-06-29-16-04.pdf",
        "4429_0_2": "Linha_4429_160161_R_2022-06-29-16-16.pdf",
        "4429_0_1": "Linha_4429_160404_G_2022-06-29-16-16.pdf",
        "4430_0_1": "Linha_4430_160029_G_2022-06-29-16-19.pdf",
        "4430_0_2": "Linha_4430_160434_R_2022-06-29-16-19.pdf",
        ...
        "4437_0_2": "Linha_4437_160067_R_2022-06-29-16-14.pdf",
        "4437_0_1": "Linha_4437_160683_G_2022-06-29-16-14.pdf",
        "4438_0_2": "Linha_4438_160347_R_2022-06-29-16-06.pdf",
        "4438_0_1": "Linha_4438_160690_G_2022-06-29-16-06.pdf",
        "4439_0_2": "Linha_4439_160067_R_2022-06-29-16-15.pdf",
        "4439_0_1": "Linha_4439_160691_G_2022-06-29-16-15.pdf",
        ...
        "4443_0_1": "Linha_4443_160704_G_2022-06-29-16-21.pdf",
        "4443_0_2": "Linha_4443_160726_R_2022-06-29-16-21.pdf",
...
function initSvgMap() {
    ...
    e.forEach((function (o) {
        const e = o.getAttribute("data-target"), i = document.querySelectorAll("rect[data-target=" + e + "]"),
            ...
            _ = s(p), d = (c = e, DATA.find((a => a.id == c)));
        var c;
        !function (o, e, i) {
...
function listaAgentes() {
    ...
    a += "</select></div>", a += '<div id="agentes" style="display:none"></div>', document.querySelector("#divAgentes").innerHTML = a, document.querySelector("#municipioAgentes").onchange = function () {
        ...
        a in diretorio_pontos_navegante && a in diretorio_espacos_navegante ? o += '<div class="titulo">EspaÃ§os e pontos naveganteÂ®</div>' : a in diretorio_pontos_navegante && !(a in diretorio_espacos_navegante) ? o += '<div class="titulo">Pontos naveganteÂ®</div>' : !(a in diretorio_pontos_navegante) && a in diretorio_espacos_navegante && (o += '<div class="titulo">EspaÃ§os naveganteÂ®</div>'), a in diretorio_pontos_navegante && (o += '<table id="tablePontoNavegante">', o += "<tr>", o += "    <th>Ponto naveganteÂ®</th>", o += `    <td>${diretorio_pontos_navegante[a][1]}<br>${diretorio_pontos_navegante[a][2]}</td>`, o += `    <td>${diretorio_pontos_navegante[a][3].replace(", ", "<br>")}</td>`, o += "</tr>"), a in diretorio_espacos_navegante && diretorio_espacos_navegante[a].forEach((a => {
            o += "<tr>", o += "    <th>EspaÃ§o naveganteÂ®</th>", o += `    <td>${a[0]}</td>`, o += `    <td>${a[1]}</td>`, o += "</tr>"
        })), o += "</table>", o += "<br>", o += '<div class="titulo">Rede de Agentes</div>', o += '<table id="tableAgentes">', freguesia_anterior = diretorio_agentes[a][3], diretorio_agentes[a].forEach((a => {
            let e = a[0], i = a[1], n = a[2], s = a[3];
            o += "<tr class='agente'>", s != freguesia_anterior ? (o += `    <th class="freguesia" style="border-top: 1px solid #ffdd00;">${s}</th>`, o += `    <td class="nome" style="border-top: 1px solid #ffdd00;">${e}</td>`, o += `    <td class="morada" style="border-top: 1px solid #ffdd00;">${i}, ${n}, ${s}</td>`) : (o += '    <th class="freguesia"></th>', o += `    <td class="nome">${e}</td>`, o += `    <td class="morada">${i}, ${n}, ${s}</td>`), freguesia_anterior = s, o += "</tr>"
